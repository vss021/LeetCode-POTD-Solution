...
int main() {
    ...
    cout << "Enter group assignments for each item:" << endl;
    for (int i = 0; i < n; i++) {
        cout << "Item " << i << " belongs to group (0 to " << m - 1 << " or -1 if none): ";
        cin >> group[i];
    }

    cout << "Enter dependencies between items (for each item, enter a list of items it depends on, separated by spaces):" << endl;
    for (int i = 0; i < n; i++) {
        cout << "Dependencies for Item " << i << ": ";
        int numDependencies;
        cin >> numDependencies;
        beforeItems[i].resize(numDependencies);
        for (int j = 0; j < numDependencies; j++) {
            cin >> beforeItems[i][j];
        }
    }

    // Call the sortItems function to obtain the sorted order
    vector<int> result = solution.sortItems(n, m, group, beforeItems);

    // Print the sorted order
    cout << "Sorted Order of Items: ";
    for (int item : result) {
        cout << item << " ";
    }
    cout << endl;
